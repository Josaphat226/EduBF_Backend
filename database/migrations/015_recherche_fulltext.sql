ALTER TABLE documents ADD COLUMN recherche_vecteur tsvector;
...
BEGIN
  ...
  NEW.recherche_vecteur := to_tsvector('french',
    ...
    coalesce(NEW.annee::text, '')
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_maj_recherche
  BEFORE INSERT OR UPDATE ON documents
  FOR EACH ROW EXECUTE FUNCTION maj_recherche_vecteur();

CREATE INDEX idx_documents_recherche ON documents USING GIN(recherche_vecteur);